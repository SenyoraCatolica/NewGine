...
//bounding Box
void GameObject::CreateBoundingBox(const ResourceMesh* mesh)
{
	originalAABB.SetNegativeInfinity();
	originalAABB.Enclose((float3*)mesh->mesh->vertices, mesh->mesh->num_vertices);
}
...
bool GameObject::GetBoundingBoxActive()
{
	return boundingbox_active;
}

void GameObject::SetBoundingBoxActive(bool active)
{
	boundingbox_active = active;
}
...
void GameObject::DrawBoundingBox()
{
	...
	for (uint i = 0; i < 12; i++)
	{
		glVertex3f(originalAABB.Edge(i).a.x, originalAABB.Edge(i).a.y, originalAABB.Edge(i).a.z);
		glVertex3f(originalAABB.Edge(i).b.x, originalAABB.Edge(i).b.y, originalAABB.Edge(i).b.z);
	}
	glEnd();
