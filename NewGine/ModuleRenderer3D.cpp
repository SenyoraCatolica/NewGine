...
{
	App->console->AddMessage("Creating 3D Renderer context");
	bool ret = true;
	...
	if (context == NULL)
	{
		App->console->AddMessage("OpenGL context could not be created! SDL_Error: %s\n", SDL_GetError());
		ret = false;
	}
	...
	if (ret == true)
	{
		...
		if (VSYNC && SDL_GL_SetSwapInterval(1) < 0)
			App->console->AddMessage("Warning: Unable to set VSync! SDL Error: %s\n", SDL_GetError());
		...
		{
			App->console->AddMessage("Error initializing OpenGL! %s\n", gluErrorString(error));
			ret = false;
		}
...
{
	App->console->AddMessage("Destroying 3D Renderer");
